...
class MainWindow(QtWidgets.QMainWindow):
    ...
    def save_template(self):
        dic = {'DICOM_tag': [], 'File_information': [], 'Custom_plugins': [], 'Version' : __version__}
        ...
        for index in range(self.ui.listWidget.count()):
            ...
            elif custom_widget.comboBoxAttributeChoice.currentText() == AttributeOptions.FILE_INFORMATION.value:
                text = custom_widget.comboBoxFileOption.currentText()
                dic['File_information'].append(text)
            elif custom_widget.comboBoxAttributeChoice.currentText() == AttributeOptions.CUSTOM_PLUGIN.value:
                ...
                try:
                    ...
                    if 'File_information' in dic:
                        for file_option in dic['File_information']:
                            self.add_new_list_widget(attribute_type=AttributeOptions.FILE_INFORMATION,
                                                     combo_box_text=file_option)
                    if 'Custom_plugins' in dic:
                        for plugin_name in dic['Custom_plugins']:
                            if plugin_name not in [plugin.name for plugin in self.plugin_manager.getAllPlugins()]:
                                msg_box = QMessageBox()
                                msg_box.setWindowTitle("Warning")
                                msg_box.setText(
                                    'A required custom plugin (' + plugin_name + ") isn't installed and will be skipped")
                                msg_box.setIcon(QMessageBox.Warning)
                                msg_box.exec()
                            else:
                                self.add_new_list_widget(attribute_type=AttributeOptions.CUSTOM_PLUGIN,combo_box_text=plugin_name)
                except KeyError:
                    msg_box = QMessageBox()
                    msg_box.setWindowTitle("Error")
                    msg_box.setText(
    ...
    def add_new_list_widget(self, checked=False, default_text='', attribute_type=AttributeOptions.DICOM_TAG, combo_box_text=None):
        ...
        if attribute_type == AttributeOptions.DICOM_TAG:
            ...
            file_info_index = custom_widget.comboBoxFileOption.findText(combo_box_text)
            ...
            plugin_index = custom_widget.comboBoxPluginOption.findText(combo_box_text)
            if plugin_index >= 0:
                custom_widget.comboBoxPluginOption.setCurrentIndex(plugin_index)
    ...
    def do_analysis(self):
        ...
        for index in range(self.ui.listWidget.count()):
            ...
            if custom_widget.comboBoxAttributeChoice.currentText() == AttributeOptions.FILE_INFORMATION.value:
            ...
            elif custom_widget.comboBoxAttributeChoice.currentText() == AttributeOptions.DICOM_TAG.value:
                # Handle DICOM tags
                text = custom_widget.lineEdit.text()
                try:
                    if text == '':
                ...
                if plugin_name != '':
                    header_custom_plugins += self.plugin_manager.getPluginByName(plugin_name).plugin_object.column_headers()
                    selected_plugins.append(plugin_name)
            else:
                raise NotImplementedError

        csv_header = (header_file_info + header_DICOM + header_custom_plugins)[0:-1]  # Remove the last comma
        ...
        self.create_csv.emit(self.ui.labelOutputFile.text(), self.ui.labelFolderToAnalysePath.text(), csv_header, dicom_tags,file_attributes,selected_plugins)
...
class AnalyseAndOutputDataThread(QObject):
    ...
    def run(self, output_file, folder_to_analyse, header, dicom_tags, file_attributes,custom_plugins):
        with open(output_file, 'w') as f:
            f.write(header + '\n')
            system_location = os.path.dirname(os.path.realpath(__file__))
            plugin_locations = [os.path.join(system_location, 'Plugins')]
            # We make a new plugin manager here to insure they are running on the new thread
            plugin_manager = PluginManager()
            plugin_manager.setPluginPlaces(plugin_locations)
            ...
            for dirpath, _, filenames in os.walk(folder_to_analyse):
                for filename in filenames:
                    ...
                    #  If it isn't a valid DICOM file or we can't load the file, we'll just skip over it
                    ...
                    for plugin_name in custom_plugins:
                        plugin = plugin_manager.getPluginByName(plugin_name).plugin_object
                        output_line += plugin.generate_values(full_path,ds)
...
class CustomListWidget(QtWidgets.QWidget):
    def __init__(self,parent=None,plugin_list=list()):
        ...
        self.comboBoxPluginOption = QComboBox()
        self.comboBoxPluginOption.addItems(plugin_list)
        self.comboBoxPluginOption.setVisible(False)
        ...
        self.layout.addWidget(self.comboBoxPluginOption)
    ...
    def change_item_type(self,selection):
        ...
        elif selection == AttributeOptions.FILE_INFORMATION.value:
            self.comboBoxFileOption.setVisible(True)
            self.comboBoxPluginOption.setVisible(False)
            self.lineEdit.setVisible(False)
            self.lineEdit.setText('')
        elif selection == AttributeOptions.CUSTOM_PLUGIN.value:
            self.comboBoxFileOption.setVisible(False)
            self.comboBoxPluginOption.setVisible(True)
            self.lineEdit.setVisible(False)
            self.lineEdit.setText('')
        else:
